...
export default function ChatInterface() {
  ...
  return (
    <>
      {/* Chat button */}
      <motion.button
        className="fixed bottom-6 right-6 z-40 h-16 w-16 rounded-full flex items-center justify-center shadow-lg will-change-transform"
        style={{
          background: "linear-gradient(45deg, #1e40af, #3b82f6)",
                    ...
                    {isListening && audioLevel > 0.1 && (
                      <>
                        {[...Array(8)].map(
                          (
                            _,
                            i, // Reduced from 12 to 8 for better performance
                          ) => (
                            <motion.div
                              key={`wave-${i}`}
                              ...
                              animate={{
                                x: [0, Math.cos((i * Math.PI) / 4) * (80 + audioLevel * 40)],
                                y: [0, Math.sin((i * Math.PI) / 4) * (80 + audioLevel * 40)],
                                opacity: [1, 0],
                              }}
                              transition={{
                                duration: 1 + audioLevel * 0.5, // Slightly faster animation
                                repeat: Number.POSITIVE_INFINITY,
                                delay: i * 0.1,
                              }}
                            />
